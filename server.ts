...
// When cert files exist (generated by mkcert), serve over HTTPS so phone/tablet
// browsers allow camera access. Falls back to plain HTTP for localhost-only dev.
const CERT_KEY  = join(process.cwd(), 'certs', 'key.pem');
const CERT_CERT = join(process.cwd(), 'certs', 'cert.pem');
const useHttps  = existsSync(CERT_KEY) && existsSync(CERT_CERT);

// When HTTPS, bind to 0.0.0.0 so phones on the same network can reach it
const hostname = useHttps ? '0.0.0.0' : 'localhost';
...
app.prepare().then(() => {
  ...
  const httpServer = useHttps
    ? createHttpsServer({ key: readFileSync(CERT_KEY), cert: readFileSync(CERT_CERT) }, requestHandler)
    : createHttpServer(requestHandler);

  if (useHttps) {
    console.log('\n🔒 HTTPS mode enabled!');
    console.log(`📱 Open on your phone: https://192.168.101.22:${port}`);
    console.log(`💻 Local:              https://localhost:${port}`);
  } else {
    console.log(`\n🌐 HTTP mode (localhost only)`);
    console.log(`   Run: node scripts/setup-https.mjs   to enable HTTPS for phone testing`);
